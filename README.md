...
2. Nómbralo, por ejemplo, `alcanzar-web`.
...
## 8. 🍎 Problemas comunes en iOS

Al ejecutar el juego en iPhone/iPad pueden aparecer algunos problemas específicos de Safari/WebKit.

### ⌨️ El teclado virtual no aparece

**Problema:** al introducir el nombre, responder una adivinanza o utilizar la consola, el teclado puede no aparecer.

**Solución:** el `focus()` del `<input>` utilizado por `teclado_movil.py` debe ejecutarse directamente después de una acción del usuario, como pulsar un botón. Evitar retrasos (`setTimeout`) antes de solicitar el foco.

---

### 👆 Los controles táctiles no responden en alguna zona

**Problema:** alguna parte de la pantalla puede no detectar los toques.

**Solución:** comprobar que los elementos HTML auxiliares no estén interceptando los eventos. En especial:
...
También comprobar que `#infobox`, `#transfer` y `#pyconsole` no estén colocados por encima del canvas cuando no se utilizan.

---

### 🔄 Los botones se desplazan al girar el dispositivo

**Problema:** después de cambiar entre vertical y horizontal, los botones pueden quedar descolocados.

**Solución:** recalcular su posición después de cada cambio de orientación y actualizar el tamaño del canvas mediante:

```javascript
actualizar_aspecto_canvas(w, h)
```

---

### 📐 El juego aparece deformado o las coordenadas táctiles no coinciden

**Problema:** el tamaño visual del canvas puede no coincidir con la resolución interna de Pygame.

**Solución:** mantener sincronizados el tamaño real de `pygame.display.set_mode()` y el tamaño CSS del canvas. Ejecutar `actualizar_aspecto_canvas()` después de cada cambio de resolución.

---

### 🔊 La música o los sonidos no comienzan

**Problema:** iOS puede bloquear la reproducción automática de audio.

**Solución:** iniciar el sistema de sonido después de una interacción del usuario, por ejemplo al pulsar el botón **JUGAR**.

---

### 📱 Los botones quedan demasiado cerca del borde

**Problema:** en algunos iPhone, el notch, Dynamic Island o la barra inferior de gestos pueden ocupar parte de la pantalla.

**Solución:** mantener un margen de seguridad respecto a los bordes y evitar colocar botones importantes directamente en las esquinas.

---

### ⏸️ El juego falla al volver después de bloquear el iPhone

**Problema:** iOS puede suspender temporalmente la página cuando se bloquea el dispositivo o se cambia de aplicación.

**Solución:** probar que el juego continúa correctamente al volver y, si algún componente deja de funcionar, reinicializar el audio, temporizadores o elementos afectados.

---

### 🌐 Pantalla blanca o juego que no carga

**Problema:** la aplicación puede no conseguir cargar correctamente la página de GitHub Pages.

**Solución:** comprobar primero que la URL funciona directamente en Safari del iPhone y que existe conexión a Internet. Si funciona en Safari pero no dentro de la aplicación, revisar la configuración del empaquetador Web → iOS.

> **Recomendación:** antes de generar la aplicación iOS, probar siempre el juego directamente desde Safari en un iPhone. Si funciona correctamente en Safari, será mucho más fácil localizar cualquier problema posterior del empaquetado.
